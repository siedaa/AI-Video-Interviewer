...
| 2 — LangGraph + HITL | In progress | src/graph.py (6 nodes + 2 conditional edges incl. missing-GitHub branch + SqliteSaver on checkpoint.db, msgpack allowlist for schemas), run_graph.py CLI with approve/edit/reject. Tested approve, edit->reapprove, reject, github-override interrupt, and crash-resume; resume bug fixed (Command(resume=...) instead of re-invoking initial input). Clean edit-flow (correct qid field new-value syntax) never fully re-verified after a Groq rate limit hit — still outstanding, needs a re-test. |
| 3 — Guardrails | Not started | |
| 4 — Realtime voice | Done | LiveKit Agents + Gemini Live (model pinned to gemini-2.5-flash-native-audio-preview-12-2025 as hardcoded default — bare alias timed out on WS handshake, diagnosed via isolated test script scripts/test_gemini_live.py). Agent joins room via explicit dispatch, greets candidate by name from question_plan.json, discloses it's an AI. Barge-in tuned (InterruptionOptions: min_duration=0.15, backchannel_boundary=(0,0), mode="vad") — default settings were too sluggish (~1s+ before reacting). Interrupt latency measured live across multiple sessions: sub-100ms to 624ms, well under 1200ms target. Transcript logging to output/transcript.json with token-overlap heuristic for stage/node tracking; interrupt_latency.json writes immediately per-event (not just at clean session close) for crash-safety. One transient Gemini Live WebSocket disconnect observed mid-session (1006 abnormal closure, keepalive timeout) — auto-recovered via built-in plugin retry logic; noted as a known limitation. Fixed a live bug where a corrupted model string ("...12-2025QARTSS" — stray characters appended, root cause traced and removed) caused Gemini Live connection to fail with a 1008 policy violation. Also fixed a race condition: if the participant disconnects during session startup, generate_reply() and avatar setup could crash with "AgentSession isn't running" — added liveness checks (_session_live() helper checking room connection + session._is_closing()/_activity) before both, now exits gracefully instead of crashing. |
| 5 — Avatar | Done | 2D lip-sync avatar: 3 programmatically-generated PNG mouth states (closed/mid/open, transparent 512x512), composited onto a video frame at ~15fps via PIL, published as a LiveKit SOURCE_CAMERA video track. Mouth state selected by real-time RMS amplitude of the agent's outgoing audio (fast-attack/slow-release peak detection). room.html updated to render the incoming video track (was audio-only before). Known limitation: mouth-sync has a startup lag — stays closed for the first portion of the agent's speech before syncing correctly; documented as a known limitation rather than blocking further phases. Also observed: Gemini's STT occasionally transcribes clear English speech in Devanagari script under some acoustic conditions — a model quirk, not a code issue. |

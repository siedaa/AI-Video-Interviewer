...
| Phase | Status | Notes |
|---|---|---|
| 0 — Scaffolding | Done | Repo skeleton, `.venv`, `.env.example`, verify_keys.py; all 4 key checks pass (Gemini/Groq/GitHub/LiveKit OK). Commit `22a51f1`. |
| 1 — Prep pipeline | Done | jd_parser/resume_parser/github_agent/question_planner + run_prep.py; 12 questions with 4 github-grounded citing real repos/files. Commit `5e612da`. |
| 2 — LangGraph + HITL | Done | 6 nodes + 2 conditional edges (route_github, route_approval) + SqliteSaver checkpointer; approve/edit/reject tested; checkpoint-resume bug fixed (Command(resume=...) + msgpack allowlist). Commit `eac96a7`. |
| 3 — Guardrails | Done | banned_questions (8 categories) + evidence_check (no-quote-no-score) with pytest tests. Commits `0d917ea`, `0eb9f44`. |
| 4 — Realtime voice | Done | LiveKit + Gemini Live (pinned `-12-2025` model), barge-in tuned to `min_duration=0.15`, transcript.json logging, latency measured 23.7–624ms; fixed model-corruption (1008), AgentSession close race, browser autoplay AudioContext block. |
| 5 — Avatar | Done | 2D amplitude-driven lip-sync (3 PNG mouth states @ ~15fps, SOURCE_CAMERA track); room.html video rendering added. Commits `0887095`, `ec5f242`. |
| 6 — Scoring + MCP | Done | scorer with evidence-check enforcement (`653422d`); MCP server 5 tools stdio + Claude Desktop proof (`d4eefd7`, `2f44bcb`). |
| 7 — Evals | Done | 5 personas scored (strong 4.6 > bluffer 3.6 > nervous 3.4 > average 2.6 > weak 1.8); Bluffer misranking documented honestly, not tuned away. Commit `e6292e3`. |
| 8 — Docs + submission | In progress | Docs finalized (README/ARCHITECTURE/SUBMISSION/ITERATION_NOTES) + report generator added (`src/agents/report_generator.py`, chained into scorer CLI → `output/report.pdf`). Blocked on: 8-min raw recording + videos, repo visibility for grading, real candidate + consent. |
